...
//
HGCalModuleTreeReader::HGCalModuleTreeReader(const std::string& tree_name,
                                             const std::vector<std::string>& filenames,
                                             unsigned int num_channels)
  : Emulator(num_channels), chain_(tree_name.c_str()) {
  
  for (const auto& filename : filenames) chain_.Add(filename.c_str());
  ...
  for (long long i = 0; i < chain_.GetEntries(); i++) {
    ...
    if (data_.count(key) == 0) data_[key] = ERxInput();

    // check if chip already exists
    ERxId_t erxKey((uint8_t)event.chip, (uint8_t)event.half);
    if (data_[key].count(erxKey) == 0) data_[key][erxKey] = ERxData();
...
ECONDInput HGCalModuleTreeReader::next() {
  ...
  if (it_data_ == data_.end())
    throw cms::Exception("HGCalModuleTreeReader")
      << "Insufficient number of events were retrieved from input tree to proceed with the generation of emulated events.";
