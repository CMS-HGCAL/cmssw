...
namespace test {
  ...
  public:
    ...
    void analyze(edm::StreamID sid, const Event& e, const EventSetup& c) const override {
      if(usePhase2_) analyzePhase2(e,c);
      else analyzePhase1(e,c);
    }
  ...
  private:
    ...
    void analyzePhase2(const Event& e, const EventSetup& c) const {
      ...
      for(const auto& it : rawdata->map()) {
          ...
          auto fedid = it.first;
          if(!feds_.empty() && feds_.find(fedid) == feds_.end()) continue;
          
          auto offset = it.second.first;
          auto size = it.second.second;
          out << "FED# " << std::setw(4) << fedid << " " << std::setw(8) << size << " bytes  offset=" << std::setw(8) << offset;
