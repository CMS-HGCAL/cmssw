...
// Porbably make a buffer at some point.
...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  struct HGCalDigiMLCalibrationKernel_module_info{
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  HGCALSoACMMLDeviceCollection::View mlsoa,
                                  HGCalDigiDevice::ConstView digis,
                                  HGCalModuleIndexerDevice::ConstView module_indexer) const {
      int nch_per_erx = 37
      for (auto idx : uniform_elements(acc, module_indexer.metadata().size())) {
        auto it = moduleIndexer.typecodeMap()[i];
        uint32_t fedid = it.second.first;
        uint32_t imod = it.second.second;
        auto nErx = moduleIndexer.getNumERxs(fedid, imod);
      }
    }
  };

  struct HGCalDigiMLCalibrationKernel_digis_info{
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  HGCALSoACMMLDeviceCollection::View mlsoa,
                                  HGCalDigiDevice::ConstView digis) const {
      int nch_per_erx = 37 // This will be fixed forever.
      int nErx = 12 // This ikjjkjks not fixed and must be looked up for each digi comes from a module.
      for (auto idx : uniform_elements(acc, digis.metadata().size())) {
        auto indexinfo = denseIndexInfo_view[idx];
        uint32_t chIdx = indexinfo.chNumber();
        double cmsum = digi.cm();
        float mean_sub_channel_index = cmIdx - (nch_per_erx * nErx - 1.0)/2.0; // Need to get the mean part form the module.
        mlsoa.msubchidx()[idx] = mean_sub_channel_index;
        int erxIdc = chIdx / nch_per_erx;
        mlsoa.msuberxidx()[idx] = erxIdc - (nErx - 1.0)/2.0; // Need to get the mean part form the module.
        mlsoa.cellfrac()[idx] = 0.5; // Requires a look up.
        ...
        if (chStartIdxWithinERX == 8 || chStartIdxWithinERX == 17 || chStartIdxWithinERX == 19 || chStartIdxWithinERX == 28) {
          mlsoa.msubunconnectedch()[idx] = 1; // This is an unconnected channel, handle accordingly
        ...
        for (int i = 0; i < 12; ++i) {
          if i < nErx {
            mlsoa.cmsum()(idx, i) = 1.0; // Need to look up how to do this.
          } else {
            mlsoa.cmsum()(idx, i) = 0.0; // No ERX there so set to 0.
          }
      }
    }
  };
  ...
  HGCalSoARecHitsDeviceCollection HGCalRecHitCalibrationAlgorithms::calibrate(
    ...
    LogDebug("HGCalMLCalibrationAlgorithms") << "\n\nINFO -- Start of calibrate\n\n" << std::endl;
    ...
    LogDebug("HGCalMLCalibrationAlgorithms") << "\n\nINFO -- Copying the modules indexer to the device\n\n" << std::endl;
    auto const nmodules = host_module_indexer.view().metadata().size();
    HGCalModuleDevice device_module_indexer(queue, nmodules);
    alpaka::memcpy(queue, device_module_indexer.buffer(), host_module_indexer.const_buffer());

    LogDebug("HGCalRecHitCalibrationAlgorithms")
        << "\n\nINFO -- Allocating MLInputs buffer" << std::endl;
    HGCalSoACMMLDeviceCollection device_MLSOA(queue, ndigis);

    // number of items per group
    uint32_t items = n_threads_;
    // use as many groups as needed to cover the whole problem
    // I think we use the n.of modules.
    uint32_t groups = divide_up_by(nmodules, items);
    // map items to
    ...
    alpaka::exec<Acc1D>(queue,
                        grid,
                        HGCalDigiMLCalibrationKernel_module_info{},
                        device_MLSOA.view(),
                        device_digis.const_view(),
                        device_module_indexer.const_view());
    alpaka::exec<Acc1D>(queue,
                        grid,
